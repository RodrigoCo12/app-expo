...
export default function Entradas() {
  ...
  // ✅ FUNCIÓN ACTUALIZADA: fetchEntradas con mejor manejo de refresh
  const fetchEntradas = async (pageNum = 1, refresh = false) => {
    // Verificar si hay token antes de hacer la petición
    if (!token) {
      setLoading(false);
    ...
    try {
      ...
      // ✅ CAMBIO: Solo aplicar filtro de fecha si no es un refresh sin filtros activos
      if (fechaInicioFiltro && fechaFinFiltro && !(refresh && !filtroActivo)) {
        const fechaInicio = new Date(fechaInicioFiltro);
        const fechaFin = new Date(fechaFinFiltro);
      ...
      // ✅ CAMBIO MEJORADO: Manejo más limpio de actualización de entradas
      if (refresh || pageNum === 1) {
        setEntradas(data.entradas);
      } else {
        // Para carga incremental, evitar duplicados usando Map
        const entradasMap = new Map();

        // Agregar entradas existentes
        entradas.forEach((entrada) => {
          entradasMap.set(entrada._id, entrada);
        });

        // Agregar nuevas entradas (sobrescribiendo duplicados)
        data.entradas.forEach((entrada) => {
          entradasMap.set(entrada._id, entrada);
        });

        setEntradas(Array.from(entradasMap.values()));
      }
  ...
  // ✅ NUEVA FUNCIÓN: Refresh manual que obtiene las entradas más recientes
  const handleRefresh = () => {
    // Si hay filtros activos, mantenerlos y recargar
    if (filtroActivo) {
      fetchEntradas(1, true);
    } else {
      // Si no hay filtros activos, obtener TODAS las entradas sin filtro de fecha
      fetchEntradas(1, true);
    }
  };
  ...
  const mostrarTodasLasEntradas = () => {
    ...
    setFechaFinFiltro(null);
    setFiltroDefault(false);
    setFiltroActivo(false); // ✅ CAMBIO: Marcar como sin filtros activos
    // Forzar recarga inmediata sin filtros
    setTimeout(() => {
      fetchEntradas(1, false);
    }, 100);
  };
    ...
    <View style={styles.container}>
      <FlatList
        ...
        refreshControl={
          <RefreshControl
            refreshing={refreshing}
            onRefresh={handleRefresh} // ✅ CAMBIO: Usar la nueva función handleRefresh
            colors={[COLORS.primary]}
            tintColor={COLORS.primary}
          />
        }
